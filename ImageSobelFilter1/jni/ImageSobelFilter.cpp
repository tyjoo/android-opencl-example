...
extern "C" {
    ...
    JNIEXPORT jstring JNICALL Java_com_white_imagesobelfilter_nativeSobelFilter_sobelFilter(
            JNIEnv* env, jobject thiz, jstring imagePath,jstring outPath)
    {
        char *clImagePath = (char *) (env)->GetStringUTFChars(imagePath, 0);
        string out_path=(env)->GetStringUTFChars(outPath,0);
        //读取图像clImagePath
        const char* imagename = clImagePath;
        IplImage* inputImg = cvLoadImage(imagename);
        LOGE("---> image loaded,%d,%d",inputImg->width,inputImg->height);
        CvSize size;
        ...
        LOGE("------->%s","start save image....");
        //保存图像
        cvSaveImage((out_path+"/Sobel_input_gray.jpg").c_str(), inputImg_gray);
        cvSaveImage((out_path+"/cpu_sobel.jpg").c_str(), cpu_outputImg);
        cvSaveImage((out_path+"/gpu_sobel.jpg").c_str(), gpu_outputImg);
        cvSaveImage((out_path+"/differenceImg.jpg").c_str(), differenceImg);
